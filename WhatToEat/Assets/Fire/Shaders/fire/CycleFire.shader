    ...
    SubShader {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                i.uv.x = i.uv.x - frac(i.uv.x * _Size) / _Size + (1/(2*_Size));
                i.uv.y = i.uv.y - frac(i.uv.y * _Size) / _Size + (1/(2*_Size));
                
                clip(rsqrt( (po.x - i.uv.x) * (po.x - i.uv.x) + (po.y - i.uv.y) * (po.y - i.uv.y)) - _Size * 2.2);
